...
def record_data(dir,audio):
    ...
    end_time = start_time + 59 #audio.audio_length()
    ideal_seconds = 0.1
    ...
    while start_time < end_time:
        ...
        elif keyboard.is_pressed("1"): # uhhhh
            filler = 1
        elif keyboard.is_pressed("2"): # ummmm
            filler = 2
        elif keyboard.is_pressed("3"): # like
            filler = 3
...
for audio in range(len(audio_files)):
    ...
    #Inital Variables
    # folder = audio_file
    dir = path + audio_file  # Folder with frames of audio we're pulling from
    ...
    if automatic_manual == 1:
        ...
        time.sleep(1)
        print("Go!")
        filler_tracker = record_data(dir, audio)
